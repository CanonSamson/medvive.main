...
// Handle different auth actions based on the dynamic route parameter
  ...
  switch (action) {
    ...
    default:
      return NextResponse.json(
        { error: 'Invalid auth action', success: false },
        { status: 400 }
      )
  }
}

// Extract your current POST logic into this function
async function handleSendOTP (request: NextRequest): Promise<NextResponse> {
  try {
...
// Extract your current PATCH logic into this function
async function handleVerifyOTP (request: NextRequest): Promise<NextResponse> {
  try {
    const body = await request.json()
...
async function handleResendOTP (request: NextRequest): Promise<NextResponse> {
  // Reuse the same logic as handleSendOTP for resending
  return handleSendOTP(request)
}
